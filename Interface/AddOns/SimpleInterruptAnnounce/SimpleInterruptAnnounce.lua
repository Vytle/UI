...
function frame:COMBAT_LOG_EVENT_UNFILTERED(timeStamp, subEvent, _, _, sourceName, sourceFlags, _, _, destName, _, destRaidFlags, spellID, _, _, extraSpellID)
	...
	if sourceName == GetPlayerName() then
		...
		-- Announce interrupt
		self:AnnounceInterrupt(config.own, string.format('Interrupted %s%s %s', GetRaidIconString(raidIcon, config.own), StringOwns(destName or '?'), GetSpellLinkCached(extraSpellID)))
	elseif bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) ~= 0 then
		-- If own is off for this config, return
		if config.own == 'off' then return end

		-- Announce interrupt
		self:AnnounceInterrupt(config.own, string.format('My %s interrupted %s%s %s', sourceName or '?', GetRaidIconString(raidIcon, config.own), StringOwns(destName or '?'), GetSpellLinkCached(extraSpellID)))
	else
		...
		if (inRaid and bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_RAID) ~= 0) or (inGroup and bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_PARTY) ~= 0) then
			...
			self:AnnounceInterrupt(config.other, string.format('%s interrupted %s%s %s', name, GetRaidIconString(raidIcon, config.other), StringOwns(destName or '?'), GetSpellLinkCached(extraSpellID)))
		end
	end
end
...
function frame:AnnounceInterrupt(mode, msg)
	if mode == 'self' then
		self:Print(msg)
	else
