...
i.time:SetFont('Fonts\\ARIALN.ttf', cfg.buffFontSize, 'THINOUTLINE')
...
time:SetPoint("TOP", 0, 20)
time:SetTextColor(255, 255, 255)
...
if f.name and f.rank then
  ...
  if name and (not f.caster or (f.caster and caster == f.caster)) and (not f.match_spellid or (f.match_spellid and spID == tmp_spellid)) then
    ...
    if time < 10 then
      ...
      f.iconframe.time:SetTextColor(255, 255, 255)
    end
