local nibPointDisplay = LibStub("AceAddon-3.0"):GetAddon("nibPointDisplay")
local db
...
local function GetOptions()
	if not options then
		options = {
			name = "nibPointDisplay",
			handler = nibPointDisplay,
			type = "group",
			args = {
				globalsettings = {
				...
				args = {
					...
					enabled = {
						...
						set = function(info, value) 
							...
							if not value then
								nibPointDisplay:DisablePointDisplay(ic, tid)
							else
								nibPointDisplay:EnablePointDisplay(ic, tid)
							end
						end,
						order = 20,
					},
					...
					copy = {
						...
						set = function(info, value)
							...
							if FromTID ~= tid then
								nibPointDisplay:CopyAllSettings(db[FromIC].types[FromTID], db[ic].types[tid], FromIC, ic, FromTID, tid, FromNum, it)
					...
					bgpanel = {
						...
						args = {
							enabled = {
								...
								set = function(info, value) db[ic].types[tid].bgpanel.enabled = value nibPointDisplay:UpdateBGPanelTextures() end,
								order = 10,
							},
							size = {
								type = "group",
							...
							border = {
								...
								args = {
									...
									edgesize = {
										...
										set = function(info, value) db[ic].types[tid].bgpanel.border.edgesize = value; nibPointDisplay:UpdateBGPanelTextures(); end,
										order = 30,
									},
									color = {
										type = "color",
							...
							opacity = {
								...
								args = {
									incombat = {
										...
										set = function(info, value) db[ic].types[tid].combatfader.opacity.incombat = value; nibPointDisplay:UpdateCombatFader(); end,
										order = 10,
									},
									hurt = {
										type = "range",
										...
										set = function(info, value) db[ic].types[tid].combatfader.opacity.hurt = value; nibPointDisplay:UpdateCombatFader(); end,
										order = 20,
									},
									target = {
										type = "range",
										...
										set = function(info, value) db[ic].types[tid].combatfader.opacity.target = value; nibPointDisplay:UpdateCombatFader(); end,
										order = 30,
									},
									outofcombat = {
										type = "range",
										...
										set = function(info, value) db[ic].types[tid].combatfader.opacity.outofcombat = value; nibPointDisplay:UpdateCombatFader(); end,
										order = 40,
									},
								},
							},
				...
				BarOpts[BarID] = {
					...
					args = {
						copy = {
							...
							set = function(info, value)
								if value ~= i then
									nibPointDisplay:CopyBarSettings(db[ic].types[tid].bars[value], db[ic].types[tid].bars[i])
									nibPointDisplay:GetTextures()
									nibPointDisplay:UpdateBGPanelTextures()
									nibPointDisplay:UpdatePosition()
									nibPointDisplay:UpdatePoints("ENABLE")
								...
								empty = {
									...
									args = {
										...
										edgesize = {
											...
											set = function(info, value) db[ic].types[tid].bars[i].border.empty.edgesize = value; nibPointDisplay:UpdatePoints("ENABLE"); end,
											order = 30,
										},
										color = {
											type = "color",
								...
								full = {
									...
									args = {
										...
										edgesize = {
											...
											set = function(info, value) db[ic].types[tid].bars[i].border.full.edgesize = value; nibPointDisplay:UpdatePoints("ENABLE"); end,
											order = 30,
										},
										colors = {
											type = "group",
						...
						spark = {
							...
							args = {
								enabled = {
									...
									set = function(info, value) db[ic].types[tid].bars[i].spark.enabled = value; nibPointDisplay:UpdatePoints("ENABLE"); end,
									order = 10,
								},
								note = {
									type = "description",
									name = "Note: You will need to use your own image to get the spark effect as nibPointDisplay does not include one.",
									order = 20,
								},
								position = {
									name = "Position",
...
-- Copy All Settings
function nibPointDisplay:CopyAllSettings(FromTable, ToTable, FromIC, ToIC, FromTID, ToTID, FromNum, ToNum)
...
-- Copy Bar Settings
function nibPointDisplay:CopyBarSettings(FromTable, ToTable)
	if not FromTable or not ToTable then return false end
	for i,v in pairs(FromTable) do
		if type(FromTable[i]) == "table" then
			for i2,v2 in pairs(FromTable[i]) do
	...
	if not intoptions then
		intoptions = {
			...
			args = {
				note = {
					type = "description",
					name = "You can access the nibPointDisplay options by typing: /nibpd",
					order = 10,
				},
				openoptions = {
					type = "execute",
...
function nibPointDisplay:OpenOptions()
	if not options then nibPointDisplay:SetUpOptions() end
	LibStub("AceConfigDialog-3.0"):Open("nibPointDisplay")
end

function nibPointDisplay:ChatCommand(input)
	nibPointDisplay:OpenOptions()
end

function nibPointDisplay:ConfigRefresh()
	db = self.db.profile
end

function nibPointDisplay:SetUpInitialOptions()
	-- Chat commands
	self:RegisterChatCommand("nibpd", "ChatCommand")
	self:RegisterChatCommand("nibpointdisplay", "ChatCommand")
	
	-- Interface panel options
	LibStub("AceConfig-3.0"):RegisterOptionsTable("nibPointDisplay-Int", GetIntOptions)
	self.optionsFrame = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("nibPointDisplay-Int", "nibPointDisplay")
end

function nibPointDisplay:SetUpOptions()
	db = self.db.profile
	...
	LibStub("AceConfig-3.0"):RegisterOptionsTable("nibPointDisplay", options)
	LibStub("AceConfigDialog-3.0"):SetDefaultSize("nibPointDisplay", 800, 600)
end